...
double wrap_phase(double phase_s, double period_s) {
  while (phase_s > 0.45 * period_s) {
    phase_s -= period_s;
  }
  while (phase_s < -0.45 * period_s) {
    phase_s += period_s;
...
double dco_freq_hz(int code) {
  struct Point {
    int code;
    double mhz;
  };

  constexpr Point table[] = {
      {0, 50.955942},   {64, 55.205750},  {128, 60.174879},
      {192, 66.031451}, {255, 72.479371},
  };
...
int main(int argc, char **argv) {
  ...
  if (xyce_initialize(&xyce, static_cast<int>(xyce_argv.size()),
                      xyce_argv.data()) != 1) {
  ...
  DlfModel dlf(args);
  const double target_hz = dco_freq_hz(args.target_code);
  ...
  for (int cycle = 0; cycle < args.cycles; ++cycle) {
    ...
    const double fdco = dco_freq_hz(code_before);

...
module tb_pll_25mhz_mode_controller;
    ...
    task check_mode_outputs;
        ...
        begin
            ...
            if (dlf_ext_data !== {expected_dco_code, 2'b00})
                $fatal(1, "%0s DLF seed mismatch: got %0d expected %0d",
                       label, dlf_ext_data, {expected_dco_code, 2'b00});
            if (dlf_ki !== 8'd16)
    ...
    initial begin
        ...
        check_mode_outputs("reset", 16'd100, 8'd4, 6'd20, 8'd93);
        ...
        mode_select = 2'd3;
        #1;
        check_controls("before mode change clock", 1'b1, 1'b0, 1'b0, 1'b1);
        check_mode_outputs("before mode change clock", 16'd300, 8'd12, 6'd4, 8'd90);
        ...
        check_mode_outputs("disabled", 16'd400, 8'd16, 6'd2, 8'd76);

        load_and_track_mode(2'd0, 16'd100, 8'd4, 6'd20, 8'd93);

        $display("PASS: 25 MHz PLL mode controller sequencing");
        $finish;
    end
endmodule

...
if (( DLF_KP_WIDTH < 1 || DLF_KP_WIDTH > 8 )); then
    echo "DLF_KP_WIDTH must be in the range 1..8, got $DLF_KP_WIDTH" >&2
    exit 1
fi
...
yosys -p "
    ...
    chparam -set DLF_KP_WIDTH $DLF_KP_WIDTH IntegerPLL_DigitalCore;
...
echo "Wrote $BUILD_DIR/IntegerPLL_DigitalCore_sky130.v (DLF_FRAC_WIDTH=$DLF_FRAC_WIDTH DLF_KP_WIDTH=$DLF_KP_WIDTH DLF_ACQ_BOOST_SHIFT=$DLF_ACQ_BOOST_SHIFT DLF_ACQ_BOOST_AFTER=$DLF_ACQ_BOOST_AFTER DLF_ACQ_RAIL_BOOST=$DLF_ACQ_RAIL_BOOST DLF_ACQ_FORCE_RAIL_CODE=$DLF_ACQ_FORCE_RAIL_CODE DLF_UPDATE_ON_PLLOUT=$DLF_UPDATE_ON_PLLOUT DLF_PROP_RAIL_GUARD=$DLF_PROP_RAIL_GUARD DCO_CONTROL_REGISTERED=$DCO_CONTROL_REGISTERED DCO_COARSE_BITS=$DCO_COARSE_BITS)"

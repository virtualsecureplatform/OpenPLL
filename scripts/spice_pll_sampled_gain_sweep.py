...
def run_combo(root, args, step_lsb, prop_lsb, sample_delay_ps, edge_sigma_rad, initial_phase):
    combo_name = (
        f"step{value_slug(step_lsb)}_"
        f"prop{value_slug(prop_lsb)}_"
        f"delay{value_slug(sample_delay_ps)}_"
        f"edge{value_slug(edge_sigma_rad)}_"
        ...
        rows.append(
            {
                ...
                "dlf_step_lsb": step_lsb,
                "dlf_prop_lsb": prop_lsb,
                "sample_delay_ps": sample_delay_ps,
                "max_step_ps": args.max_step_ps,
                "edge_sigma_rad": edge_sigma_rad,
...
def summarize(rows):
    ...
    for row in rows:
        ...
        combo = combos.setdefault(
            ...
            {
                "sweep_combo": combo_name,
                "dlf_step_lsb": row.get("dlf_step_lsb", ""),
                "dlf_prop_lsb": row.get("dlf_prop_lsb", ""),
                "sample_delay_ps": row.get("sample_delay_ps", ""),
                "max_step_ps": row.get("max_step_ps", ""),
                "edge_sigma_rad": row.get("edge_sigma_rad", ""),
    ...
    return sorted(
        ...
        key=lambda row: (
            ...
            float(row["dlf_step_lsb"]),
            float(row["dlf_prop_lsb"]),
            float(row["sample_delay_ps"]),
        ),
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Sweep sampled PLL-loop SPICE surrogate gain and aperture settings."
    )
    parser.add_argument("--dlf-step-lsb-values", default="2.5,3.0,3.5")
    parser.add_argument("--dlf-prop-lsb-values", default="0")
    parser.add_argument("--sample-delay-ps-values", default="0,150,300")
    parser.add_argument("--edge-sigma-rad-values", default="0.03")
    parser.add_argument("--initial-dco-phase-cycles-values", default="0")
    parser.add_argument("--cases", default="low_start,high_start")
    parser.add_argument(
        "--loop-model",
        choices=("sampled", "sampled_latched"),
        default="sampled",
        help="Sampled SPICE loop surrogate to run.",
    )
    parser.add_argument("--sim-time-us", type=float, default=20.0)
    ...
    parser.add_argument("--loop-current-sign", type=float, default=1.0)
    parser.add_argument("--clock-sharpness", type=float, default=20.0)
    ...
    parser.add_argument(
        "--build-dir",
        default=str(Path(__file__).resolve().parents[1] / "build" / "spice_pll_sampled_gain_sweep"),
    )
    parser.add_argument(
        "--require-pass",
        action="store_true",
    ...
    combos = [
        (step_lsb, prop_lsb, sample_delay_ps, edge_sigma_rad, initial_phase)
        for step_lsb in parse_float_list(args.dlf_step_lsb_values)
        for prop_lsb in parse_float_list(args.dlf_prop_lsb_values)
        for sample_delay_ps in parse_float_list(args.sample_delay_ps_values)
        for edge_sigma_rad in parse_float_list(args.edge_sigma_rad_values)
    ...
    rows_path = args.build_dir / "sampled_gain_sweep.csv"
    summary_path = args.build_dir / "sampled_gain_summary.csv"
    ...
    for row in summary:
        print(
            f"step={row['dlf_step_lsb']} prop={row['dlf_prop_lsb']} "
            f"delay_ps={row['sample_delay_ps']} "
            f"edge_sigma={row['edge_sigma_rad']} pass_both={row['pass_both']} "
    ...
    if args.require_pass and not any(row["pass_both"] for row in summary):
        print("no sampled-loop gain setting passed both rails", file=sys.stderr)
        return 1
    return 0
